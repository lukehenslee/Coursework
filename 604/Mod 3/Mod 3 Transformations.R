...
# ---------------------------------------------------------------------
# Analytical: take logarithm of both sides to transform to linearity,
#  that is, show that log(S) is a linear function of log(A)!
#  (insert equations as comments). 

# Start with:
#                  S = c * A^z

# Then take logarithm of both sides and simplify...
#
#
#
...
# Plot fitted model in several ways:
# Visualize fit on log-log scale
plot(log(S) ~ log(A))
abline(SA.fit)

# Visualize increase in log(S) by area:
library(visreg)
visreg(SA.fit)

# And, finally, back-transform the y-axis 
# to actual species numbers (richness):
visreg(SA.fit, trans=exp, add=T)
# Note that this does NOT apply a bias correction
# Note the asymmetrical confidence band
# Note that residuals are not included because
#    of the transformation of the response variable
